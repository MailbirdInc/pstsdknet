...
namespace pstsdk.test.mocks.MockPropBags
{
    ...
    {        
        ...
        {
            if (!MockConstants.PropertyTypeValues.ContainsKey(id))
                throw new PstSdkException();

            return MockConstants.PropertyTypeValues[id];
        }

        public bool PropertyExists(PropId id)
        {
            return MockConstants.PropertyExistValues.ContainsKey(id);
        }

        public uint PropertySize(PropId id)
        {
            if (!MockConstants.PropertySizeValues.ContainsKey(id))
                throw new PstSdkException(String.Format("PropId: {0}", id));

            return MockConstants.PropertySizeValues[id];
        }

        public byte[] ReadProperty(PropId id)
        {
            if (!MockConstants.ReadPropertyValues.ContainsKey(id))                            
                throw new PstSdkException(String.Format("PropId: {0}", id));

            return MockConstants.ReadPropertyValues[id];
        }        

        public Stream OpenPropertyStream(PropId id)
        {
            if (!MockConstants.PropertyStreamValues.ContainsKey(id))                            
                throw new PstSdkException();

            return MockConstants.PropertyStreamValues[id];
        }
